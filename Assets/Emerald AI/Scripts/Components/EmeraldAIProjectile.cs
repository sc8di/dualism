...
namespace EmeraldAI.Utility
{
    public class EmeraldAIProjectile : MonoBehaviour
    {
        ...
        {
            if (UseRandomizedTrajectory == Yes_No.Yes)
            {
                m_TrajectoryOffset = new Vector3(Random.Range(TrajectoryXOffsetMin, TrajectoryXOffsetMax), 
                    Random.Range(TrajectoryYOffsetMin, TrajectoryYOffsetMax), Random.Range(TrajectoryZOffsetMin, TrajectoryZOffsetMax));
            }
            else
            {
                m_TrajectoryOffset = Vector3.zero;
            }
        }
        */
    }
