                ...
                {                    
                        ...
                        !EmeraldComponent.GettingHit && EmeraldComponent.CurrentBlockingState == EmeraldAISystem.BlockingState.NotBlocking)
                    {
                        ...
                        if (!EmeraldComponent.TargetObstructed && EmeraldComponent.WeaponTypeRef == EmeraldAISystem.WeaponType.Ranged || 
                            ...
                            EmeraldComponent.DestinationAdjustedAngle <= EmeraldComponent.MaxDamageAngle && m_TargetHeight <= EmeraldComponent.AttackHeight)
                        {
                            float angle = EmeraldComponent.TargetAngle();                           
                            if (EmeraldComponent.WeaponTypeRef == EmeraldAISystem.WeaponType.Ranged && angle > EmeraldComponent.MaxFiringAngle || EmeraldComponent.CurrentAggroHits >= EmeraldComponent.TotalAggroHits)
                            {
                                EmeraldComponent.AttackTimer = 0;
                                return;
                            }
                            ...
                            EmeraldComponent.AttackTimer = 0;
                            float RandomOffset = Random.Range(0, 0.5f); //Add a small random offset to the attack speed to prevent attacks from firing simultaneously
