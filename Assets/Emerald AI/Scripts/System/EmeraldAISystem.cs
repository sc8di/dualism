...
namespace EmeraldAI
{
    ...
    public class EmeraldAISystem : MonoBehaviour
    {
        #region Variables
        //Abilities
        public List<EmeraldAIAbility> OffensiveAbilitiesList = new List<EmeraldAIAbility>();
        public List<EmeraldAIAbility> SupportAbilitiesList = new List<EmeraldAIAbility>();
        public List<EmeraldAIAbility> SummoningAbilitiesList = new List<EmeraldAIAbility>();
        ...
        public int Ability1AnimationIndex;
        public int Ability1Odds = 70;
        public int Ability2AnimationIndex;
        public int Ability3AnimationIndex;
        public int Ability3Odds = 30;
        ...
        public AnimationClip RangedDeath1Animation, RangedDeath2Animation, RangedDeath3Animation;
        ...
        public void Wander ()
        {
            ...
            {
                if (m_NavMeshAgent.remainingDistance <= StoppingDistance && m_NavMeshAgent.velocity.magnitude != 0)
                {
        ...
        void CreateEmeraldProjectile()
        {
            if (CurrentTarget != null)
            {
                ...
                {
                    if (AttackAnimationNumber == 1 && OffensiveAbilitiesList.Count > 0) //Offensive
                    {
                        OnAttackEvent.Invoke();
                        EmeraldAIAbility m_EmeraldAIAbility = OffensiveAbilitiesList[Random.Range(0, OffensiveAbilitiesList.Count)];
                    ...
                    else if (AttackAnimationNumber == 2 && SupportAbilitiesList.Count > 0 && !HealingCooldownActive) //Support
                    {
                        EmeraldAIAbility m_EmeraldAIAbility = SupportAbilitiesList[Random.Range(0, SupportAbilitiesList.Count)];
                        ...
                        else if (m_EmeraldAIAbility.AbilityType == EmeraldAIAbility.AbilityTypeEnum.Support && m_EmeraldAIAbility.SupportType == EmeraldAIAbility.SupportTypeEnum.Instant)
                        {
                            ...
                            if (CombatTextSystem.Instance.m_EmeraldAICombatTextData.PlayerOnly == EmeraldAICombatTextData.PlayerOnlyEnum.Disabled)
                            {
                                CombatTextSystem.Instance.CreateCombatTextAI(m_EmeraldAIAbility.AbilitySupportAmount, transform.position, false, true);
                            }
                        }
                    ...
                    else if (AttackAnimationNumber == 3 && SummoningAbilitiesList.Count > 0 && TotalSummonedAI < MaxAllowedSummonedAI) //Summon
                    {
                        TotalSummonedAI++;
                        EmeraldAIAbility m_EmeraldAIAbility = SummoningAbilitiesList[Random.Range(0, SummoningAbilitiesList.Count)];                       
        ...
        IEnumerator InitializeSummonedAI (EmeraldAISystem AIObject)
        {
            ...
            AIObject.AIAttacksPlayerRef = AIAttacksPlayerRef;
                    ...
                    || WeaponTypeRef == WeaponType.Ranged && !m_SwitchingWeaponTypes)
                {
                    ...
                    if (AdjustedAngle <= MaxDamageAngle || WeaponTypeRef == WeaponType.Ranged)
                    {
                        ...
                        {                          
                            ...
                            if (TargetEmerald.CurrentBlockingState != EmeraldAISystem.BlockingState.Blocking && !TargetEmerald.IsDead)
                            {
                                EmeraldEventsManagerComponent.PlayImpactSound();
                            }
                        }
        ...
        public void Damage (int DamageAmount, TargetType? TypeOfTarget = null, Transform AttackerTransform = null, int RagdollForce = 100, bool CriticalHit = false)
        {
            ...
            if (AttackerTransform != CurrentFollowTarget && BehaviorRef == CurrentBehavior.Companion || BehaviorRef != CurrentBehavior.Companion && BehaviorRef != CurrentBehavior.Pet)
            {
                if (!AIAnimator.GetBool("Blocking") || AdjustedBlockAngle > MaxBlockAngle)
                {
                    ...
                    {
                        if (CombatTextSystem.Instance.m_EmeraldAICombatTextData.PlayerOnly == EmeraldAICombatTextData.PlayerOnlyEnum.Disabled && !IsDead)
                        {
                            CombatTextSystem.Instance.CreateCombatTextAI(DamageReceived, transform.position, CriticalHit, false);
                        }
                    }
                ...
                else if (AIAnimator.GetBool("Blocking") && CurrentBlockingState == BlockingState.Blocking && AdjustedBlockAngle <= MaxBlockAngle)
                {
                    ...
                    if (TypeOfTarget != TargetType.Player)
                    {
                        CombatTextSystem.Instance.CreateCombatTextAI((int)BlockDamage, transform.position, CriticalHit, false);
                    }
                    AIAnimator.ResetTrigger("Attack");
                    AIAnimator.SetTrigger("Hit");
                    if (CurrentlyPlayingActionAnimation != CombatIdleAnimation && !Attacking)
                    {
